...
| :--------------------------------------------------------------------------------------------------------------------------------------------- | :----------------- | :------------------------- |
| [Apple Button Element SDK with Payrails decryption](#1-apple-pay-button-element-sdk-with-payrails-decrypted-tokens)        | Low                | Low (SAQ and AOC)          |
| [Server-to-Server with Payrails decryption](#2-server-to-server-with-payrails-decrypted-tokens) | Medium             | Low (SAQ and AOC)          |
| [Server-to-Server with Merchant decryption](#3-server-to-server-with-merchant-decrypted-tokens) | High               | High (Level 1 ROC and AOC) |
...
Elements provides merchants with easy integration to Apple Pay tokenization without having the need to set up any integration with the Apple Pay SDK directly. Payrails manages all the keys needed for completing the Apple Pay session and detokenization, reducing all merchant overhead. Check out the [Apple Pay Button Element](/docs/orchestration/checkout-sdks/web-v5-legacy/elements#apple-pay-element) for a detailed guide.

For details of how to integrate this option, move to this [section](#setting-up-the-apple-pay-button-element-for-tokenization) .
...
For details of how to integrate this option, move to this [section](#2-server-to-server-with-payrails-decrypted-tokens-2).
...
For details of how to integrate this option, move to this [section](#3-server-to-server-with-merchant-decrypted-tokens-2).

You can jump to [this section](#using-apple-pay-instrument-in-instant-proxy) to learn how to pass an Apple Pay instrument in a proxy API.
...
Once the instrument has been created, it can be used in the instant proxy in your backend service. Jump to this [section](#using-apple-pay-instrument-in-instant-proxy) to read more.
...
Once the instrument has been created, it can be used in the instant proxy in your backend service. Move to the next [section](#using-apple-pay-instrument-in-instant-proxy) to read more.
...
<Warning>
  Apple Pay tokens will always contain a network token in them, always use the
  network token place holders when making a proxy request for an apple pay
  instrument.
</Warning>
