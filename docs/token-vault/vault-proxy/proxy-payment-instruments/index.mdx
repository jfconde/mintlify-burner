...
If the card was initially stored with the correct [authorization flags](doc:authorization-flags), you should not need the security code to authorize a payment. However, we provide this option in case you have one of the following use cases:
...
To obtain the value of the `encryptedSecurityCode`, you can use our client-side SDK function called `encryptCardData`. Check the documentation for it [here](doc:tokenize-cards-with-client-side-encryption).
