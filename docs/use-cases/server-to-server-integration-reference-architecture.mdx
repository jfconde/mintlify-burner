...
The server-to-server architecture is the Payrails integration pattern where your backend owns the payment lifecycle. Your backend creates [executions](doc:executions), calls authorize against them, handles [3-D Secure](doc:3d-secure) redirects, and reconciles state from [webhooks](doc:receive-notifications). The Payrails Web SDK appears only as a thin layer on the frontend, hosting [Secure Fields](doc:secure-fields) — the iframes that tokenize card data before the data reaches your servers.
...
<Note>
  If your frontend can host the [Drop-in SDK](doc:drop-in) — the SDK that drives
  the full payment lifecycle from the client — choose that pattern instead. Use
  the Drop-in SDK when your frontend owns the customer session and Payrails
  should drive the full payment lifecycle. Use the server-to-server pattern when
  your backend owns order creation, fraud checks, and customer messaging, or
  when your tech stack cannot host a single-page application.
</Note>
...
The three dashed arrows are the exceptions to that path. The browser tokenizes card data directly with Payrails through the [Vault](doc:vault) so the PAN never reaches your servers. The PSP routes a 3-D Secure challenge to the customer's browser when the issuer requires one. Payrails sends an `executionActionCompleted` webhook to your backend once the authorization resolves.
...
Every webhook delivery carries an HMAC signature header. Verify the signature before processing the body; the [Receive notifications](doc:receive-notifications) page documents the algorithm and header name.
...
Implement `verify_payrails_signature` per the [Receive notifications](doc:receive-notifications) documentation.
...
- Review the [Secure Fields reference](doc:secure-fields) for the per-field event surface (`READY`, `CHANGE`, `FOCUS`, `BLUR`) and the styling options available on the iframes.
- Read [Notifications](/docs/resources/notifications/notifications) for the canonical HMAC signing format and the full event catalog.
- See [Payment status](doc:payment-status) for the meaning of every status value your reconciliation logic must handle, including `Unknown` (which routes to manual reconciliation, not automatic retry).
- Use [Test payments](doc:test-payments) to exercise both the frictionless and 3-D Secure paths during testing, including the Test PSP for simulating declines and challenges.
