---
description: "Korean Local Cards enable South Korea-based customers to pay using all locally issued cards from South Korean card issuers (such as Shinhan, Hyundai, and Samsung Card). When a customer makes a payment, they are redirected to Stripe's local processor partner to select their card issuer and authenticate the payment, then redirected back to your site after authorization.  This allows merchants to accept card payments from the majority of South Korean consumers without a local entity."
---
## Introduction
...
2. Configure a new integration account for Korean Local Cards via **Stripe** as a Payment Service Provider.
3. Enable Korean Local Cards as a payment method in your Stripe integration configuration. In the Payrails portal, navigate to **Settings → Integrations**, select your Stripe integration instance, and enable the **Korean Local Cards** (`kr_card`) checkbox under Payment methods, then save the account.
4. Enable Korean Local Cards as a payment option in your workflow.
5. Make sure you're sending the Korean Local Cards-specific required meta fields in your requests (see Required meta fields below).
...
> **Note on amounts:** KRW is a zero-decimal currency. Amounts should be provided as whole numbers — e.g., `50000` for ₩50,000. The minimum amount is 100 KRW.
>
> **Note:** Korean Local Cards use a redirect-based flow. After calling the authorize endpoint, the customer is redirected to Stripe's local processor partner to select their card issuer and authenticate. Once confirmed, the customer will be redirected back to the `returnInfo.success` URL. Make sure to ensure your business complies with South Korean legal and tax requirements when providing goods and services to Korean customers.
