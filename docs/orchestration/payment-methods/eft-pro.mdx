---
description: "EFT Pro is an alternative payment method in South Africa, exclusively created and offered by PayU. With EFT Pro, customers can enjoy a seamless checkout experience, with a choice of fulfilling their payments instantly via bank transfer from the list of supported banks."
title: "EFT Pro"
---
# Introduction
...
2. Configure a new [integration account](/docs/orchestration/integrations/index) for EFT Pro via [PayU](/docs/orchestration/integrations/index)
3. Enable EFT Pro as a [payment option](./set-up-payment-methods).
4. Make sure you're sending the EFT Pro specific meta fields (found under Merchant configurations -> Meta fields on your Payrails dashboard and in the examples below) in your requests.

# Ways to integrate EFT Pro

## Payrails SDK

The simplest way to use EFT Pro with Payrails is to use our [drop-in](/docs/orchestration/checkout-sdks/web-v5-legacy/drop-in) in your checkout flow. With this integration type, no additional development work is required to accept payments with EFT Pro.
...
You can then make a request to our [authorize a payment endpoint](/reference/authorizeaction) with `eftPro` as the `paymentMethodCode`. See an example below:
