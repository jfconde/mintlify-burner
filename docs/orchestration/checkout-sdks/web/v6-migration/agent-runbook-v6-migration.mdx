...
- **The installed package is the contract.** When this runbook or any doc
  ...
  or in the [reference docs](/docs/orchestration/checkout-sdks/web/api-references/event-api-reference).
...
5. **Update&#x20;**`onClientInitialized`**&#x20;handler bodies** (inventory 4): the argument
   ...
   per
   [guide §3](/docs/v6-migration-guide#3-onclientinitialized-receives-the-execution-response-object).
6. **Replace event bags with&#x20;**`.on()` (inventory 5–7) using the mapping tables
   in
   [guide §4](/docs/v6-migration-guide#4-typed-on-event-api-replaces-events--callback-bags).
...
7. **Replace availability callbacks** (inventory 8) with
   ...
   ([guide §5](/docs/v6-migration-guide#5-wallet-availability-is-a-promise-not-an-event)).
8. **Migrate styling** (inventory 9–10) to `appearance` using the per-element
   mapping tables in [guide §6](/docs/v6-migration-guide#6-styles-becomes-appearance).
...
9. **Clean up types** (inventory 11) per
   [guide §7](/docs/v6-migration-guide#7-typescript-changes).
...
- **Visual sign-off** — v6 ships a redesign
  ([guide §8](/docs/v6-migration-guide#8-refreshed-visual-design)); a human must approve
...
- [Payrails class reference](/docs/orchestration/checkout-sdks/web/api-references/payrails-api-reference) — option types,
  methods, exports.
- [Events reference](/docs/orchestration/checkout-sdks/web/api-references/event-api-reference) — every event with
