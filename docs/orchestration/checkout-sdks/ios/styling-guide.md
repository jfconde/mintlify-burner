...
> **Tip:** You only need to set properties that _differ_ from the `base` state. For example, if the `focus` state should only change the border color, just set `borderColor` in `focus`.
...
| Variant     | Description                                                  |
| ----------- | ------------------------------------------------------------ |
| `.outlined` | _(Default)_ — bordered rectangle with transparent background |
