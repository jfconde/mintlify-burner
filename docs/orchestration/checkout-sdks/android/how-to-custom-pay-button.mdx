...
[`PayrailsPaymentLauncher`](/docs/sdk-api-reference#payrailspaymentlauncher).

## Prerequisites

- An active Payrails session (see [Quick Start](/docs/android-sdk-1))
- The `Session` reference returned by `Payrails.createSession(...)`
- The `com.payrails.android:checkout` dependency (see [Quick Start](/docs/android-sdk-1))
- For the encrypted-card path: a card payload **already encrypted** by your client-side
  ...
  [Why no encryption API](/docs/sdk-concepts#why-no-encryption-api))
...
Each `authorize(...)` resolves to exactly one [`ActionResult`](/docs/sdk-api-reference#actionresult),
...
[`session.getPaymentMethodConfig()`](/docs/sdk-api-reference#sessiongetpaymentmethodconfigfilter)
...
- [`PayrailsPaymentLauncher` API Reference](/docs/sdk-api-reference#payrailspaymentlauncher) — every method and factory
- [Custom UI: Launcher vs Client](/docs/sdk-concepts#custom-ui-the-launcher-and-the-internal-client) — how the launcher and the internal networking client relate
- [How to Execute a Payment Without an SDK Button (Headless)](/docs/how-to-execute-payment-headless) — the same launcher flow framed for headless/ViewModel-driven checkouts
- [`ActionResult` API Reference](/docs/sdk-api-reference#actionresult) — every terminal payment outcome
